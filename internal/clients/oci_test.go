...
func TestWithProviderMetaCacheSize(t *testing.T) {
	options := setupOptions{providerMetaCacheSize: defaultProviderMetaCacheSize}
	WithProviderMetaCacheSize(7)(&options)
	if options.providerMetaCacheSize != 7 {
		t.Fatalf("providerMetaCacheSize = %d, want 7", options.providerMetaCacheSize)
	}

	WithProviderMetaCacheSize(0)(&options)
	if options.providerMetaCacheSize != 1 {
		t.Fatalf("providerMetaCacheSize = %d, want minimum 1", options.providerMetaCacheSize)
	}
}
...
func TestValidateNoForkCredentialConfiguration(t *testing.T) {
	tests := map[string]struct {
		...
		"inline API key": {
			credentials: map[string]string{
				...
				credentialKeyPrivateKey: "inline-key",
			},
		},
		"default API key requires inline key": {
			credentials: map[string]string{},
			wantErr:     true,
		},
		"config profile is rejected even with inline key": {
			credentials: map[string]string{
				credentialKeyAuth:              "ApiKey",
				credentialKeyPrivateKey:        "inline-key",
		...
		"workload identity does not require inline key": {
			credentials: map[string]string{
				credentialKeyAuth: "OKEWorkloadIdentity",
			},
		},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			err := validateNoForkCredentialConfiguration(tc.credentials)
			if tc.wantErr && err == nil {
				t.Fatal("validateNoForkCredentialConfiguration returned nil error")
			}
			if !tc.wantErr && err != nil {
				t.Fatalf("validateNoForkCredentialConfiguration returned error: %v", err)
			}
		})
	}
}

func TestPrepareNoForkCredentialConfigurationResolvesPrivateKeyFile(t *testing.T) {
	path := filepath.Join(t.TempDir(), "key.pem")
	if err := os.WriteFile(path, []byte("first-key"), 0o600); err != nil {
		t.Fatal(err)
	}

	first := map[string]string{
		credentialKeyAuth:           "ApiKey",
		credentialKeyPrivateKeyPath: path,
	}
	if err := prepareNoForkCredentialConfiguration(first); err != nil {
		t.Fatalf("prepareNoForkCredentialConfiguration(first) error: %v", err)
	}
	if got := first[credentialKeyPrivateKey]; got != "first-key" {
		t.Fatalf("resolved private key = %q, want first-key", got)
	}
	if _, ok := first[credentialKeyPrivateKeyPath]; ok {
		t.Fatal("resolved credentials retained private_key_path")
	}
	...
	if err := prepareNoForkCredentialConfiguration(second); err != nil {
		t.Fatalf("prepareNoForkCredentialConfiguration(second) error: %v", err)
	}
	firstHash, err := providerConfigurationHash(providerConfigurationFromCredentials(first))
	if err != nil {
		t.Fatal(err)
	}
	secondHash, err := providerConfigurationHash(providerConfigurationFromCredentials(second))
	if err != nil {
		t.Fatal(err)
	}
	if firstHash == secondHash {
		t.Fatal("private key rotation did not change provider configuration hash")
	}
}

func TestPrepareNoForkCredentialConfigurationRejectsProfile(t *testing.T) {
	err := prepareNoForkCredentialConfiguration(map[string]string{
		credentialKeyAuth:              "ApiKey",
		credentialKeyPrivateKey:        "inline-key",
		credentialKeyConfigFileProfile: "DEFAULT",
	})
	if err == nil {
		t.Fatal("prepareNoForkCredentialConfiguration returned nil error")
	}
}
