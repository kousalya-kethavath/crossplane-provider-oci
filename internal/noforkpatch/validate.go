...
func ValidatePatchedTree(providerDir string) error {
	rules := []validationRule{
		...
		{
			name: "Crossplane scope coupled to OCI SDK service gate",
			paths: []string{
				"internal/provider/crossplaneservices",
			},
			pattern: regexp.MustCompile(`OciSdkEnabledServices(Map|Mu)`),
		},
	}
	...
	required := []requiredValidationRule{
		...
		{
			name:    "selective SDKv2 provider factory",
			path:    "internal/provider/provider.go",
			pattern: regexp.MustCompile(`func[[:space:]]+ProviderForResources\(resourceNames[[:space:]]+\.\.\.string\)`),
		},
		{
			name:    "public selective SDKv2 provider bridge",
			path:    "oci/provider.go",
			pattern: regexp.MustCompile(`internalprovider\.ProviderForResources\(resourceNames\.\.\.\)`),
		},
		{
			name:    "link-time Crossplane service scope",
			path:    "internal/provider/crossplaneservices/scope.go",
			pattern: regexp.MustCompile(`var[[:space:]]+Scope[[:space:]]+string`),
		},
		{
			name:    "service-scoped SDKv2 resource registration",
			path:    "internal/provider/register_resource.go",
			pattern: regexp.MustCompile(`common[[:space:]]+"github\.com/oracle/terraform-provider-oci/internal/provider/crossplaneservices"`),
		},
		{
			name:    "service-scoped SDKv2 data-source registration",
			path:    "internal/provider/register_datasource.go",
			pattern: regexp.MustCompile(`common[[:space:]]+"github\.com/oracle/terraform-provider-oci/internal/provider/crossplaneservices"`),
		},
		{
			name:    "service-scoped Framework data-source registration",
			path:    "internal/provider/register_framework_datasource.go",
			pattern: regexp.MustCompile(`common[[:space:]]+"github\.com/oracle/terraform-provider-oci/internal/provider/crossplaneservices"`),
		},
		{
			name:    "Crossplane scope independent from OCI SDK enabled-services map",
			path:    "internal/provider/crossplaneservices/scope.go",
			pattern: regexp.MustCompile(`return[[:space:]]+common\.CheckForEnabledServices\(service\)`),
		},
		{
