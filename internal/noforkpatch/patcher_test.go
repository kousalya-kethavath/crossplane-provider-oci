...
func TestValidateDestructivePathsRejectsUnsafeTargets(t *testing.T) {
	root := t.TempDir()
	tests := map[string]func(*Options){
		"provider directory is repository root": func(opts *Options) {
			opts.ProviderDir = root
		},
		"provider directory is source directory": func(opts *Options) {
			opts.ProviderDir = filepath.Join(root, "internal")
		},
		"state directory is outside repository": func(opts *Options) {
			opts.StateDir = filepath.Join(filepath.Dir(root), "outside-state")
		},
		"gopath is work root": func(opts *Options) {
			opts.GoPath = filepath.Join(root, ".work")
		},
		"state directory overlaps provider directory": func(opts *Options) {
			opts.StateDir = filepath.Join(opts.ProviderDir, "state")
		},
	}
	...
	for name, mutate := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if err := opts.validateForPatch(); err == nil {
				t.Fatal("validateForPatch returned nil error for unsafe destructive path")
			}
		})
	}
}

func TestValidateDestructivePathsRejectsSymlinkTraversal(t *testing.T) {
	root := t.TempDir()
	outside := t.TempDir()
	...
	if err := os.Symlink(outside, filepath.Join(workRoot, "escape")); err != nil {
		t.Skipf("cannot create symlink: %v", err)
	}

	opts := DefaultOptions(root)
	opts.ProviderVersion = "8.22.0"
	opts.ProviderDir = filepath.Join(workRoot, "escape", "terraform-provider-oci")

	if err := opts.validateForPatch(); err == nil {
		t.Fatal("validateForPatch returned nil error for symlink traversal")
	}
}

func TestValidateDestructivePathsRejectsSymlinkedWorkRoot(t *testing.T) {
	root := t.TempDir()
	outside := t.TempDir()
	if err := os.Symlink(outside, filepath.Join(root, ".work")); err != nil {
		t.Fatal(err)
	}

	opts := normalizeOptions(DefaultOptions(root))
	if err := validateDestructivePaths(opts); err == nil || !strings.Contains(err.Error(), "work root is a symbolic link") {
		t.Fatalf("validateDestructivePaths() error = %v, want symlinked work-root error", err)
	}
}
