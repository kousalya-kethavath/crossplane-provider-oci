...
`)
	mustWrite(t, root+"/internal/provider/crossplaneservices/scope.go", `
package crossplaneservices

func configure() {
	common.OciSdkEnabledServicesMap["events"] = true
}
`)
	...
	for _, want := range []string{
		...
		"Crossplane scope coupled to OCI SDK service gate",
...
`)
	mustWrite(t, root+"/oci/provider.go", `
package oci

func ProviderForResources(resourceNames ...string) (*schema.Provider, error) {
	return internalprovider.ProviderForResources(resourceNames...)
}
`)
	mustWrite(t, root+"/internal/provider/crossplaneservices/scope.go", `
package crossplaneservices
...
func CheckForEnabledServices(service string) bool {
	return common.CheckForEnabledServices(service)
}
`)
	for _, path := range []string{
		"/internal/provider/register_resource.go",
		"/internal/provider/register_datasource.go",
		"/internal/provider/register_framework_datasource.go",
	} {
		mustWrite(t, root+path, `
package provider

import common "github.com/oracle/terraform-provider-oci/internal/provider/crossplaneservices"
`)
	}
