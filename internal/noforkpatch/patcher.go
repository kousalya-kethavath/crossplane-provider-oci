...
func Clean(opts Options) error {
	...
	if err := validateDestructivePaths(opts); err != nil {
		return err
	}
	var errs []error
...
func (o Options) validateForPatch() error {
	...
	return validateDestructivePaths(o)
}

func validateDestructivePaths(opts Options) error {
	workRoot := filepath.Join(opts.RootDir, ".work")
	targets := []struct {
		name string
		path string
	}{
		{name: "provider-dir", path: opts.ProviderDir},
		{name: "state-dir", path: opts.StateDir},
		{name: "gopath", path: opts.GoPath},
	}

	for _, target := range targets {
		if err := validateDestructivePath(workRoot, target.path); err != nil {
			return fmt.Errorf("unsafe %s %q: %w", target.name, target.path, err)
		}
	}
	for i, first := range targets {
		for _, second := range targets[i+1:] {
			if pathsOverlap(first.path, second.path) {
				return fmt.Errorf("unsafe no-fork workspace: %s %q overlaps %s %q", first.name, first.path, second.name, second.path)
			}
		}
	}
	return nil
...
func validateDestructivePath(workRoot, target string) error {
	workRoot = filepath.Clean(workRoot)
	target = filepath.Clean(target)

	if info, err := os.Lstat(workRoot); err == nil {
		if info.Mode()&os.ModeSymlink != 0 {
			return fmt.Errorf("work root is a symbolic link: %s", workRoot)
		}
	} else if !errors.Is(err, os.ErrNotExist) {
		return fmt.Errorf("inspect work root %s: %w", workRoot, err)
	}

	rel, err := filepath.Rel(workRoot, target)
	if err != nil {
		return fmt.Errorf("resolve path relative to work root: %w", err)
	}
	if rel == "." || rel == ".." || strings.HasPrefix(rel, ".."+string(filepath.Separator)) {
		return fmt.Errorf("path must be a child of %s", workRoot)
	}
	...
	for _, part := range strings.Split(rel, string(filepath.Separator)) {
		...
		if info.Mode()&os.ModeSymlink != 0 {
			return fmt.Errorf("path traverses symbolic link %s", current)
		}
	}
	return nil
}
