...
import (
	...
	serviceapis "github.com/oracle/provider-oci/internal/apis/runtime/email"
	...
	clustercontroller "github.com/oracle/provider-oci/internal/controller/cluster/email"
	"github.com/oracle/provider-oci/internal/features"
	namespacedcontroller "github.com/oracle/provider-oci/internal/controller/namespaced/email"
)
...
func main() {
	var (
		app              = kingpin.New(filepath.Base(os.Args[0]), "OCI support for Crossplane - email Service.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-oci-email"))
	if debugValue {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-oci-email",
		LeaderElectionResourceLock: resourcelock.LeasesResourceLock,
		LeaseDuration:              func() *time.Duration { d := 60 * time.Second; return &d }(),
		RenewDeadline:              func() *time.Duration { d := 50 * time.Second; return &d }(),
	})
	kingpin.FatalIfError(err, "Cannot create controller manager")
	kingpin.FatalIfError(serviceapis.AddToSchemes.AddToScheme(mgr.GetScheme()), "Cannot add email APIs to scheme")
	kingpin.FatalIfError(apiextensionsv1.AddToScheme(mgr.GetScheme()), "Cannot add apiextensions APIs to scheme")
	kingpin.FatalIfError(resolverapis.BuildScheme(serviceapis.AddToSchemes), "Cannot register the email APIs with the API resolver's runtime scheme")
	...
	o := tjcontroller.Options{
		...
		Provider: config.GetProviderForRuntime("email"),
		OperationTrackerStore: tjcontroller.NewOperationStore(log),
		SetupFn: clients.TerraformSetupBuilder(
			clients.WithSDKv2ResourcePredicate(config.SDKv2ResourcePredicateForRuntime("email")),
	...
	if canWatchCRD(context.Background(), cfg, log) {
		log.Info("SafeStart enabled", "service", "email")
		o.Gate = &gate.Gate[schema.GroupVersionKind]{}

		kingpin.FatalIfError(setupGatedControllers(mgr, o), "Cannot setup gated email controllers")
		kingpin.FatalIfError(customresourcesgate.Setup(mgr, o.Options), "Cannot setup custom resource gate controller")
	} else {
		log.Info("SafeStart disabled; falling back to eager controller setup", "service", "email")
		kingpin.FatalIfError(setupControllers(mgr, o), "Cannot setup email controllers")
	}
	kingpin.FatalIfError(mgr.Start(ctrl.SetupSignalHandler()), "Cannot start controller manager")
}

func setupControllers(mgr ctrl.Manager, o tjcontroller.Options) error {
	if err := clustercontroller.Setup_email(mgr, o); err != nil {
		return err
	}
	return namespacedcontroller.Setup_email(mgr, o)
}

func setupGatedControllers(mgr ctrl.Manager, o tjcontroller.Options) error {
	if err := clustercontroller.SetupGated_email(mgr, o); err != nil {
		return err
	}
	return namespacedcontroller.SetupGated_email(mgr, o)
}
