...
func main() {
	var (
		...
		providerMetaCacheSize = app.Flag("provider-meta-cache-size", "Maximum number of configured OCI SDKv2 ProviderConfigs retained in memory.").Default("32").Envar("PROVIDER_META_CACHE_SIZE").Int()
	...
	if *providerMetaCacheSize <= 0 {
		kingpin.Fatalf("--provider-meta-cache-size must be greater than 0")
	}
	...
	if *metricsBindAddress != "0" {
		...
		crmetrics.Registry.MustRegister(clients.ProviderMetaCacheCollectors()...)
