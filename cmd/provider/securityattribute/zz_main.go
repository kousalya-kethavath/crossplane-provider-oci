...
import (
	...
	serviceapis "github.com/oracle/provider-oci/internal/apis/runtime/securityattribute"
	...
	clustercontroller "github.com/oracle/provider-oci/internal/controller/cluster/securityattribute"
	"github.com/oracle/provider-oci/internal/features"
	namespacedcontroller "github.com/oracle/provider-oci/internal/controller/namespaced/securityattribute"
)
...
func main() {
	...
	log := logging.NewLogrLogger(zl.WithName("provider-oci-securityattribute"))
	if debugValue {
	...
	kingpin.FatalIfError(serviceapis.AddToSchemes.AddToScheme(mgr.GetScheme()), "Cannot add securityattribute APIs to scheme")
	kingpin.FatalIfError(apiextensionsv1.AddToScheme(mgr.GetScheme()), "Cannot add apiextensions APIs to scheme")
	kingpin.FatalIfError(resolverapis.BuildScheme(serviceapis.AddToSchemes), "Cannot register the securityattribute APIs with the API resolver's runtime scheme")
	...
	o := tjcontroller.Options{
		...
		Provider: config.GetProviderForRuntime("securityattribute"),
		OperationTrackerStore: tjcontroller.NewOperationStore(log),
		SetupFn: clients.TerraformSetupBuilder(
			clients.WithSDKv2ResourcePredicate(config.SDKv2ResourcePredicateForRuntime("securityattribute")),
	...
	if canWatchCRD(context.Background(), cfg, log) {
		log.Info("SafeStart enabled", "service", "securityattribute")
		o.Gate = &gate.Gate[schema.GroupVersionKind]{}

		kingpin.FatalIfError(setupGatedControllers(mgr, o), "Cannot setup gated securityattribute controllers")
		kingpin.FatalIfError(customresourcesgate.Setup(mgr, o.Options), "Cannot setup custom resource gate controller")
	} else {
		log.Info("SafeStart disabled; falling back to eager controller setup", "service", "securityattribute")
		kingpin.FatalIfError(setupControllers(mgr, o), "Cannot setup securityattribute controllers")
	}
	kingpin.FatalIfError(mgr.Start(ctrl.SetupSignalHandler()), "Cannot start controller manager")
}

func setupControllers(mgr ctrl.Manager, o tjcontroller.Options) error {
	if err := clustercontroller.Setup_securityattribute(mgr, o); err != nil {
		return err
	}
	return namespacedcontroller.Setup_securityattribute(mgr, o)
}

func setupGatedControllers(mgr ctrl.Manager, o tjcontroller.Options) error {
	if err := clustercontroller.SetupGated_securityattribute(mgr, o); err != nil {
		return err
	}
	return namespacedcontroller.SetupGated_securityattribute(mgr, o)
}
