...
import (
	...
	serviceapis "github.com/oracle/provider-oci/internal/apis/runtime/containerengine"
	...
	clustercontroller "github.com/oracle/provider-oci/internal/controller/cluster/containerengine"
	"github.com/oracle/provider-oci/internal/features"
	namespacedcontroller "github.com/oracle/provider-oci/internal/controller/namespaced/containerengine"
)
...
func main() {
	var (
		app              = kingpin.New(filepath.Base(os.Args[0]), "OCI support for Crossplane - containerengine Service.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-oci-containerengine"))
	if debugValue {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-oci-containerengine",
		LeaderElectionResourceLock: resourcelock.LeasesResourceLock,
		LeaseDuration:              func() *time.Duration { d := 60 * time.Second; return &d }(),
		RenewDeadline:              func() *time.Duration { d := 50 * time.Second; return &d }(),
	})
	kingpin.FatalIfError(err, "Cannot create controller manager")
	kingpin.FatalIfError(serviceapis.AddToSchemes.AddToScheme(mgr.GetScheme()), "Cannot add containerengine APIs to scheme")
	kingpin.FatalIfError(apiextensionsv1.AddToScheme(mgr.GetScheme()), "Cannot add apiextensions APIs to scheme")
	kingpin.FatalIfError(resolverapis.BuildScheme(serviceapis.AddToSchemes), "Cannot register the containerengine APIs with the API resolver's runtime scheme")
	...
	o := tjcontroller.Options{
		...
		Provider: config.GetProviderForRuntime("containerengine"),
		OperationTrackerStore: tjcontroller.NewOperationStore(log),
		SetupFn: clients.TerraformSetupBuilder(
			clients.WithSDKv2ResourcePredicate(config.SDKv2ResourcePredicateForRuntime("containerengine")),
	...
	if canWatchCRD(context.Background(), cfg, log) {
		log.Info("SafeStart enabled", "service", "containerengine")
		o.Gate = &gate.Gate[schema.GroupVersionKind]{}

		kingpin.FatalIfError(setupGatedControllers(mgr, o), "Cannot setup gated containerengine controllers")
		kingpin.FatalIfError(customresourcesgate.Setup(mgr, o.Options), "Cannot setup custom resource gate controller")
	} else {
		log.Info("SafeStart disabled; falling back to eager controller setup", "service", "containerengine")
		kingpin.FatalIfError(setupControllers(mgr, o), "Cannot setup containerengine controllers")
	}
	kingpin.FatalIfError(mgr.Start(ctrl.SetupSignalHandler()), "Cannot start controller manager")
}

func setupControllers(mgr ctrl.Manager, o tjcontroller.Options) error {
	if err := clustercontroller.Setup_containerengine(mgr, o); err != nil {
		return err
	}
	return namespacedcontroller.Setup_containerengine(mgr, o)
}

func setupGatedControllers(mgr ctrl.Manager, o tjcontroller.Options) error {
	if err := clustercontroller.SetupGated_containerengine(mgr, o); err != nil {
		return err
	}
	return namespacedcontroller.SetupGated_containerengine(mgr, o)
}
