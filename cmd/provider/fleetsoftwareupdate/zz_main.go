...
import (
	...
	serviceapis "github.com/oracle/provider-oci/internal/apis/runtime/fleetsoftwareupdate"
	...
	clustercontroller "github.com/oracle/provider-oci/internal/controller/cluster/fleetsoftwareupdate"
	"github.com/oracle/provider-oci/internal/features"
	namespacedcontroller "github.com/oracle/provider-oci/internal/controller/namespaced/fleetsoftwareupdate"
)
...
func main() {
	var (
		app              = kingpin.New(filepath.Base(os.Args[0]), "OCI support for Crossplane - fleetsoftwareupdate Service.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-oci-fleetsoftwareupdate"))
	if debugValue {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-oci-fleetsoftwareupdate",
		LeaderElectionResourceLock: resourcelock.LeasesResourceLock,
		LeaseDuration:              func() *time.Duration { d := 60 * time.Second; return &d }(),
		RenewDeadline:              func() *time.Duration { d := 50 * time.Second; return &d }(),
	})
	kingpin.FatalIfError(err, "Cannot create controller manager")
	kingpin.FatalIfError(serviceapis.AddToSchemes.AddToScheme(mgr.GetScheme()), "Cannot add fleetsoftwareupdate APIs to scheme")
	kingpin.FatalIfError(apiextensionsv1.AddToScheme(mgr.GetScheme()), "Cannot add apiextensions APIs to scheme")
	kingpin.FatalIfError(resolverapis.BuildScheme(serviceapis.AddToSchemes), "Cannot register the fleetsoftwareupdate APIs with the API resolver's runtime scheme")
	...
	o := tjcontroller.Options{
		...
		Provider: config.GetProviderForRuntime("fleetsoftwareupdate"),
		OperationTrackerStore: tjcontroller.NewOperationStore(log),
		SetupFn: clients.TerraformSetupBuilder(
			clients.WithSDKv2ResourcePredicate(config.SDKv2ResourcePredicateForRuntime("fleetsoftwareupdate")),
	...
	if canWatchCRD(context.Background(), cfg, log) {
		log.Info("SafeStart enabled", "service", "fleetsoftwareupdate")
		o.Gate = &gate.Gate[schema.GroupVersionKind]{}

		kingpin.FatalIfError(setupGatedControllers(mgr, o), "Cannot setup gated fleetsoftwareupdate controllers")
		kingpin.FatalIfError(customresourcesgate.Setup(mgr, o.Options), "Cannot setup custom resource gate controller")
	} else {
		log.Info("SafeStart disabled; falling back to eager controller setup", "service", "fleetsoftwareupdate")
		kingpin.FatalIfError(setupControllers(mgr, o), "Cannot setup fleetsoftwareupdate controllers")
	}
	kingpin.FatalIfError(mgr.Start(ctrl.SetupSignalHandler()), "Cannot start controller manager")
}

func setupControllers(mgr ctrl.Manager, o tjcontroller.Options) error {
	if err := clustercontroller.Setup_fleetsoftwareupdate(mgr, o); err != nil {
		return err
	}
	return namespacedcontroller.Setup_fleetsoftwareupdate(mgr, o)
}

func setupGatedControllers(mgr ctrl.Manager, o tjcontroller.Options) error {
	if err := clustercontroller.SetupGated_fleetsoftwareupdate(mgr, o); err != nil {
		return err
	}
	return namespacedcontroller.SetupGated_fleetsoftwareupdate(mgr, o)
}
