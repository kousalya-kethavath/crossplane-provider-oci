...
import (
	...
	serviceapis "github.com/oracle/provider-oci/internal/apis/runtime/managedkafka"
	...
	clustercontroller "github.com/oracle/provider-oci/internal/controller/cluster/managedkafka"
	"github.com/oracle/provider-oci/internal/features"
	namespacedcontroller "github.com/oracle/provider-oci/internal/controller/namespaced/managedkafka"
)
...
func main() {
	var (
		app              = kingpin.New(filepath.Base(os.Args[0]), "OCI support for Crossplane - managedkafka Service.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-oci-managedkafka"))
	if debugValue {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-oci-managedkafka",
		LeaderElectionResourceLock: resourcelock.LeasesResourceLock,
		LeaseDuration:              func() *time.Duration { d := 60 * time.Second; return &d }(),
		RenewDeadline:              func() *time.Duration { d := 50 * time.Second; return &d }(),
	})
	kingpin.FatalIfError(err, "Cannot create controller manager")
	kingpin.FatalIfError(serviceapis.AddToSchemes.AddToScheme(mgr.GetScheme()), "Cannot add managedkafka APIs to scheme")
	kingpin.FatalIfError(apiextensionsv1.AddToScheme(mgr.GetScheme()), "Cannot add apiextensions APIs to scheme")
	kingpin.FatalIfError(resolverapis.BuildScheme(serviceapis.AddToSchemes), "Cannot register the managedkafka APIs with the API resolver's runtime scheme")
	...
	o := tjcontroller.Options{
		...
		Provider: config.GetProviderForRuntime("managedkafka"),
		OperationTrackerStore: tjcontroller.NewOperationStore(log),
		SetupFn: clients.TerraformSetupBuilder(
			clients.WithSDKv2ResourcePredicate(config.SDKv2ResourcePredicateForRuntime("managedkafka")),
	...
	if canWatchCRD(context.Background(), cfg, log) {
		log.Info("SafeStart enabled", "service", "managedkafka")
		o.Gate = &gate.Gate[schema.GroupVersionKind]{}

		kingpin.FatalIfError(setupGatedControllers(mgr, o), "Cannot setup gated managedkafka controllers")
		kingpin.FatalIfError(customresourcesgate.Setup(mgr, o.Options), "Cannot setup custom resource gate controller")
	} else {
		log.Info("SafeStart disabled; falling back to eager controller setup", "service", "managedkafka")
		kingpin.FatalIfError(setupControllers(mgr, o), "Cannot setup managedkafka controllers")
	}
	kingpin.FatalIfError(mgr.Start(ctrl.SetupSignalHandler()), "Cannot start controller manager")
}

func setupControllers(mgr ctrl.Manager, o tjcontroller.Options) error {
	if err := clustercontroller.Setup_managedkafka(mgr, o); err != nil {
		return err
	}
	return namespacedcontroller.Setup_managedkafka(mgr, o)
}

func setupGatedControllers(mgr ctrl.Manager, o tjcontroller.Options) error {
	if err := clustercontroller.SetupGated_managedkafka(mgr, o); err != nil {
		return err
	}
	return namespacedcontroller.SetupGated_managedkafka(mgr, o)
}
