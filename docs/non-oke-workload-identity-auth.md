...
Use this authentication method when `provider-oci` runs in a Kubernetes cluster and should authenticate to OCI with a projected Kubernetes service account token instead of an OCI API key.
...
```text
system:serviceaccount:crossplane-system:crossplane-oci-provider-wif
```
