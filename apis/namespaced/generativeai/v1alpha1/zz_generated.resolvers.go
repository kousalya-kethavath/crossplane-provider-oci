...
func (mg *AgentAgent) ResolveReferences( // ResolveReferences of this AgentAgent.
	...
	for i3 := 0; i3 < len(mg.Spec.ForProvider.LlmConfig); i3++ {
		for i4 := 0; i4 < len(mg.Spec.ForProvider.LlmConfig[i3].RoutingLlmCustomization); i4++ {
			for i5 := 0; i5 < len(mg.Spec.ForProvider.LlmConfig[i3].RoutingLlmCustomization[i4].LlmSelection); i5++ {
				{
					m, l, err = apisresolver.GetManagedResource("aidocument.oci.m.upbound.io", "v1alpha1", "Model", "ModelList")
					if err != nil {
						return errors.Wrap(err, "failed to get the reference target managed resource and its list for reference resolution")
					}
					rsp, err = r.Resolve(ctx, reference.NamespacedResolutionRequest{
...
func (mg *AgentAgentEndpoint) ResolveReferences(ctx context.Context, c client.Reader) error {
	...
	for i3 := 0; i3 < len(mg.Spec.ForProvider.ProvisionedCapacityConfig); i3++ {
		{
			m, l, err = apisresolver.GetManagedResource("generativeai.oci.m.upbound.io", "v1alpha1", "AgentProvisionedCapacity", "AgentProvisionedCapacityList")
			if err != nil {
				return errors.Wrap(err, "failed to get the reference target managed resource and its list for reference resolution")
			}
			rsp, err = r.Resolve(ctx, reference.NamespacedResolutionRequest{
